...
这是 OneChart 体系中的操作型工具表面。界面像一张可持续填写和复核的风险工作表：中性背景、白色数据平面、清楚的输入—结果关系，以及只在风险、收益、警示和当前选择上出现的颜色。顶部品牌入口固定使用 Futura 字标已转为矢量轮廓的正式 OneChart 横向 Lockup，浅色主题使用黑色版本，深色主题使用白色版本，不使用 HTML 文字、运行时字体、滤镜近似或另绘变体。它拒绝旧式玻璃面板、渐变、彩色卡片、胶囊按钮、emoji 和装饰阴影。
